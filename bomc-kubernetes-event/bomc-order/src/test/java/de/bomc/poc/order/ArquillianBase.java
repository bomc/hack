...
public class ArquillianBase {
    ...
    /**
     * Creates a new web archive, with cdi and the audit log as interceptor
     * enabled.
     *
     * @param webArchiveName
     *            the name of the archive to be created.
     ...
     *         interceptor enabled.
     */
    protected static WebArchive createTestArchiveWithAuditLog(final String webArchiveName) {

        return ShrinkWrap.create(WebArchive.class, webArchiveName + ".war").addClass(ArquillianBase.class)
                .addAsWebInfResource(getBeansXmlWithAuditLogInterceptor(), "beans.xml");
    }
    ...
    protected static StringAsset getBeansXmlWithPerformanceTrackingInterceptor() {
        return new StringAsset("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
                ...
                + "         'all'       - all components are processed, just like they were in Java EE 6 with the explicit beans.xml.\n"
                + "         'none'      - CDI is effectively disabled. -->\n" + "    <interceptors>\n"
                + "    <class>de.bomc.poc.order.application.basis.performance.interceptor.PerformanceTrackingInterceptor</class>"
                + " </interceptors>\n" 
                + "</beans>");
    }

    protected static StringAsset getBeansXmlWithAuditLogInterceptor() {
        return new StringAsset("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
                + "<beans xmlns=\"http://xmlns.jcp.org/xml/ns/javaee\" "
                + "     xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"\n" 
                + "     xsi:schemaLocation=\"\n"
                ...
                + "         'all'       - all components are processed, just like they were in Java EE 6 with the explicit beans.xml.\n"
                + "         'none'      - CDI is effectively disabled. -->\n" + "    <interceptors>\n"
                + "    <class>de.bomc.poc.order.application.basis.log.interceptor.AuditLogInterceptor</class>"
                + " </interceptors>\n" 
                + "</beans>");
    }
    ...
    protected URI buildUri(final String webArchiveName) throws URISyntaxException {
        ...
        final int port = wildflyDefaultHttpPort
                + Integer.parseInt(System.getProperty(ArquillianBase.WILDFLY_PORT_OFFSET_SYSTEM_PROPERTY_KEY));

        // Build the base Url.
        return new URI(ArquillianBase.DEFAULT_SCHEME, null,
                System.getProperty(ArquillianBase.WILDFLY_BIND_ADDRESS_SYSTEM_PROPERTY_KEY), port,
                "/" + webArchiveName + JaxRsActivator.APPLICATION_PATH, null, null);
    }
}
