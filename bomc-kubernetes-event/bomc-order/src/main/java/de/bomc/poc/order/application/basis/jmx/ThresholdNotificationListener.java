...
/**
 * A listener for exceeded thresholds. NOTE: This listener is ApplicationScoped.
 * 
    ...
    public void handleNotification(final Notification notification, final Object handback) {
        ...
        if (notification instanceof Notification) {
            ...
            if (mNotification.getType().equals(PerformanceTracking.EXCEEDED_LIMIT_NOTIFICATION_TYPE)) {
                ...
                this.thresHoldEvent.fire(new ThresholdEvent(notification.getUserData().toString()));
            }
        }
        ...
        if (notification instanceof MonitorNotification) {
            ...
            if (mNotification.getType().equals(MonitorNotification.THRESHOLD_ERROR)) {
                ...
                // not correct.
                this.thresHoldEvent.fire(new ThresholdEvent(notification.getUserData().toString()));
            } else if (mNotification.getType().equals(MonitorNotification.THRESHOLD_HIGH_VALUE_EXCEEDED)) {
                //
                // Notification type denoting that the
                // observed attribute has exceeded the
                // threshold high value.
                this.thresHoldEvent.fire(new ThresholdEvent(notification.getUserData().toString()));
            } else if (mNotification.getType().equals(MonitorNotification.THRESHOLD_LOW_VALUE_EXCEEDED)) {
                //
                // Notification type denoting that the
                // observed attribute has exceeded the
                // threshold low value.
