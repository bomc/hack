...
public class OrderAggregate implements Serializable {
	 ...
	 * handle commands. When an <code>OrderCreateCommand</code> is dispatched,
	 ...
	 */
	@CommandHandler
	public OrderAggregate(final OrderCreateCommand fileOrderCommand) {
		...
		apply(new OrderCreateEvent(fileOrderCommand.getOrderId(), fileOrderCommand.getProductInfo()));
	}
	...
	public void on(final OrderCreateEvent orderFiledEvent) {
		LOGGER.debug(LOG_PREFIX + "on#EventSourcingHandler [orderFiledEvent=" + orderFiledEvent + ", " + this.toString()
				+ "]");
