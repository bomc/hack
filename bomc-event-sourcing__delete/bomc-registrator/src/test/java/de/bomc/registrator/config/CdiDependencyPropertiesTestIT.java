package de.bomc.registrator.config;
...
import de.bomc.registrator.category.IntegrationTestIT;
import de.bomc.registrator.config.ConfigurationPropertiesConstants;
import de.bomc.registrator.config.producer.PropertiesConfiguratorProducer;
...
public class CdiDependencyPropertiesTestIT extends ArquillianBase {
	...
	public void test010_v1_readProperties_Pass() throws URISyntaxException {
		...
		final String topic = this.cdiAmbigMockEJB.getKafkaProperties()
				.getProperty(ConfigurationPropertiesConstants.KAFKA_BOMC_TOPIC_PROPERTY_KEY);
		...
		this.logger.debug(LOG_PREFIX + "test010_v1_readProperties_Pass [topic=" + topic + ", version=" + version + "]");

		assertThat(topic, equalTo("bomc-app_topic"));
