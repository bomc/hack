 ...
 */
public class JvmMetrics extends AbstractMBean implements JvmMetricsMBean {

    private static final String LOG_PREFIX = "JvmMetrics${symbol_pound}";
    ...
    public JvmMetrics() {
        ...
        this.attributesSystemMap.put(MBEAN_ATTRIBUTE_OS_PROCESS_CPU_LOAD_KEY, MBEAN_ATTRIBUTE_OS_PROCESS_CPU_LOAD_VALUE);
     ...
     * @param attributeMap the attribute map (SystemCpuLoad or ProcessCpuLoad)
     * @return cpu metrics depends on given attribute list or -1.0 if metric could not determine.
     */
    private double getCpuMetrics(final Map<String, String> attributeMap) {
        Double value = -1.0;

        try {
            final AttributeList list = mbs.getAttributes(this.objectNameOS, attributeMap.keySet()
