 ...
 * </pre>
 */
package ${package}.interfaces.rest.v1.basis.overview;
...
import ${package}.interfaces.rest.v1.basis.ApiOverviewRestEndpoint;
/**
...
public class ApiOverviewRestEndpointImpl implements ApiOverviewRestEndpoint {
    ...
    private static final String LOG_PREFIX = "ApiOverviewRestEndpointImpl${symbol_pound}";
    @Inject
    @LoggerQualifier
    ...
    public Response getAvailableEndpoints(@Context final Dispatcher dispatcher) {
        this.logger.debug(LOG_PREFIX + "getAvailableEndpoints");

        final ResourceMethodRegistry registry = (ResourceMethodRegistry)dispatcher.getRegistry();

        final List<ResourceDescription> descriptions = ResourceDescription.fromBoundResourceInvokers(registry.getBounded()
                                                                                                             .entrySet());
        ...
        descriptions.forEach(description -> {
            ...
            description.getCalls()
                       .forEach(methodDescription -> {
                           ...
                           jsonObjectBuilder.add("consumes", ((methodDescription.getConsumes() == null) ? "not set" : methodDescription.getConsumes()));
                           jsonObjectBuilder.add("produces", ((methodDescription.getProduces() == null) ? "not set" : methodDescription.getProduces()));
    ...
    public static final class MethodDescription implements Serializable {
        ...
        public MethodDescription(final String method, final String fullPath, final String produces, final String consumes, final boolean deprecated) {
            this.method = method;
    ...
    public static final class ResourceDescription implements Serializable {
        ...
        private static final String LOG_PREFIX = "ApiOverviewRestEndpointImpl${symbol_pound}ResourceDescription${symbol_pound}";
        ...
        public void addMethod(final String path, final ResourceInvoker resourceInvoker) {
            if (resourceInvoker instanceof ResourceMethodInvoker) {
                ...
                calls.addAll(method.getHttpMethods()
                                   .stream()
                                   .map(verb -> new MethodDescription(verb, path, produces, consumes, method.getMethod()
        ...
        public static List<ResourceDescription> fromBoundResourceInvokers(final Set<Map.Entry<String, List<ResourceInvoker>>> bound) {
            ...
            for (Map.Entry<String, List<ResourceInvoker>> entry : bound) {
                ...
                if (aMethod.getMethod()
                           .getAnnotation(Path.class) != null) {
                    final String
                        methodEndpoint =
                        aMethod.getMethod()
                               .getAnnotation(Path.class)
                    ...
                    for (ResourceInvoker invoker : entry.getValue()) {
                        descriptions.get(basePath)
                                    .addMethod(basePath + methodEndpoint, invoker);
                    }
                } else {
                    LOGGER.warn(LOG_PREFIX + "fromBoundResourceInvokers - no methodEndpoint set to basepath. [basePath" + basePath + "]");
                }
            }
