...
public class ConfigSingletonEJB {
    ...
    private static final String LOG_PREFIX = "ConfigSingletonEJB${symbol_pound}";
    ...
    // -----------------------------------------------
    public static final String DEFAULT_SERVICE_NAME = "${artifactId}";
    ...
    public void init() {
        ...
        try {
            ...
            final String[] nodeNameParts = this.nodeName.split("${symbol_escape}${symbol_escape}.");
            ...
            this.logger.warn(LOG_PREFIX
                             + "init - reading properties from file failed! ${symbol_escape}nUsing default properties! ${symbol_escape}n[failed on="
                             + propertyFromConfigFile
                             + "] ${symbol_escape}nNOTE: Check if 'configuration.properties'-file is in deployment-artifact or node.name is set!", ex);
        }
