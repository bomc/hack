...
public class ExponentialBackOff implements BackOff {
	...
	private long initialInterval = HystrixCommand.DEFAULT_INITIAL_INTERVAL;
	private double multiplier = HystrixCommand.DEFAULT_MULTIPLIER;
	private long maxInterval = HystrixCommand.DEFAULT_MAX_INTERVAL;
	private long maxElapsedTime = HystrixCommand.DEFAULT_MAX_ELAPSED_TIME;
	private int maxRetryCount = HystrixCommand.DEFAULT_MAX_RETRY_COUNT;
	 ...
	 * @param maxRetryCount
	 *            the max retry count. -1 means retry should not be performed.
	 */
	...
	private class ExponentialBackOffExecution implements BackOffExecution {
		...
		public long nextBackOff() {
			if (ExponentialBackOff.this.maxRetryCount < 0
					|| this.currentElapsedTime >= ExponentialBackOff.this.maxElapsedTime
					|| this.retryCount == ExponentialBackOff.this.maxRetryCount) {
				return STOP;
			}
		...
		public int getRetryCount() {
			LOGGER.debug(LOG_PREFIX + "ExponentialBackOffExecution#getRetryCount [retryCount=" + this.retryCount + "]");
		...
		public String toString() {
			return LOG_PREFIX + "ExponentialBackOff [" + "currentInterval="
