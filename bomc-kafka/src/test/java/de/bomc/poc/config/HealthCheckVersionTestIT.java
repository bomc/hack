...
import de.bomc.poc.category.IntegrationTestIT;
import de.bomc.poc.config.producer.PropertiesConfiguratorProducer;
...
import de.bomc.poc.rest.endpoints.VersionEndpoint;
...
public class HealthCheckVersionTestIT extends ArquillianBase {
	...
	private static final String WEB_ARCHIVE_NAME = "kafka-poc";
	// _______________________________________________
	// Member variables.
	// -----------------------------------------------
	@Inject
	...
	public static Archive<?> createDeployment() {
		...
		webArchive.addClasses(VersionEndpoint.class, PropertiesConfiguratorProducer.class, PropertiesConfigTypeQualifier.class, PropertiesConfigTypeEnum.class, ConfigurationPropertiesConstants.class);
		...
		webArchive.addAsResource(ConfigurationPropertiesConstants.KAFKA_PROPERTIES_FILE,
				ConfigurationPropertiesConstants.KAFKA_PROPERTIES_FILE);
	...
	public void test010_v1_readVersion_Pass() throws URISyntaxException {
		...
		try {
			final ResteasyWebTarget target = client.target(this.buildUri(WEB_ARCHIVE_NAME) + "/version/current");
