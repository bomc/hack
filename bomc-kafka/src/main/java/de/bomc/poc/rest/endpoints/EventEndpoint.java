...
import de.bomc.poc.events.command.EventCommandService;
import de.bomc.poc.events.entity.EventInfo;
import de.bomc.poc.events.entity.EventType;
...
public class EventEndpoint {
	...
	@ApiOperation(notes = "bomc - add here some notes.", value = "Send event to kafka-topic.", nickname = "sendEvent", httpMethod = "GET")
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Successfully finished.", response = String.class) })
	public String sendEvent() {
		this.logger.debug(LOG_PREFIX + "sendEvent");
...
//        return Response.accepted().header(HttpHeaders.LOCATION, uri).build();
        ...
        final String jsonResponse = jsonb.toJson(new JsonResponseObject("OK"));
