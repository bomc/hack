...
public class BasicAuthMockFilter implements ContainerRequestFilter {
	...
	// Define the valid username and password, these attributes for comparison
	// are defined in the configuration.properties and in the
	// ConfigurationSingletonEJB.
	...
	public void filter(final ContainerRequestContext containerRequestContext) throws IOException {
		...
		if (authorization != null) {
			...
			if (m.matches()) {
				...
				final byte[] decoded = Base64.getDecoder().decode(credentialsEncodedBase64);
				...
				if (credentials.equals(USERNAME_TO_MATCH + ":" + PASSWORD_TO_MATCH)) {
					...
					LOGGER.debug(
							LOG_PREFIX + "filter - authentication finished successfull. [username=" + USERNAME_TO_MATCH
									+ ", password=" + StringUtils.substring(PASSWORD_TO_MATCH, 0, 2) + "***" + "]");
			...
			final Response response = Response.status(Response.Status.UNAUTHORIZED)
					.header(HttpHeaders.WWW_AUTHENTICATE, "Basic Realm=\"(-_-)\"").type("text/plain; charset=UTF-8")
					.entity("see configuration.properties for right username and password.").build();
