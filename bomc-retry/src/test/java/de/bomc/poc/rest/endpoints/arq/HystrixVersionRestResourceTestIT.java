...
public class HystrixVersionRestResourceTestIT extends ArquillianBase {
	...
	public static Archive<?> createDeployment() {
		...
		webArchive.addClasses(HystrixBootstrapSingletonEJB.class, HystrixVersionCommand.class,
				WildflyHystrixConcurrencyStrategy.class, HystrixCommandInvokerEJB.class);
		webArchive.addClasses(JaxRsActivator.class, LoggerQualifier.class, LoggerProducer.class);
		webArchive.addClasses(BackOff.class, BackOffExecution.class, ExponentialBackOff.class, RetryHandlerInterceptor.class, RetryPolicy.class, TimeoutSingletonEJB.class);
		...
		webArchive.addAsLibraries(resolver.loadPomFromFile("pom.xml")
				.resolve("org.jboss.resteasy:resteasy-jackson-provider:jar:?").withTransitivity().asFile());
		...
		webArchive.addAsLibraries(resolver.loadPomFromFile("pom.xml")
				.resolve("com.fasterxml.jackson.core:jackson-annotations:jar:?").withTransitivity().asFile());
	...
	public void test01_v1_readVersion_Pass() {
		...
		try {
			...
			if (response.getStatus() == Response.Status.OK.getStatusCode()) {
				...
				this.logger.info(LOG_PREFIX + "test01_v1_readVersion_Pass [" + hystrixDTO + "]");
			}
		} finally {
			if (response != null) {
				response.close();
